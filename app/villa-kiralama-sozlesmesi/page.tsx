...
export default function SozlesmePage() {
  ...
  const renderSections = () => {
    return sozlesmeData.sections.map((section, sectionIndex) => (
      <Card
        key={section.id}
  ...
  const renderFaq = () => {
    return (
      <Card 
        ...
        style={{ transitionDelay: `${(sozlesmeData.sections.length + 3) * 100}ms` }}
      >
        <CardHeader className="border-b border-gray-50 bg-gray-50/50">
        ...
        <CardContent className="p-6 md:p-8">
          <Accordion type="single" collapsible className="w-full">
            {sozlesmeData.faq.map((item, index) => (
  ...
  return (
    <div className="container mx-auto py-12 px-4 md:px-6">
      ...
      {/* Başlık Kartı */}
      <Card 
        className={`mb-12 border-0 bg-white shadow-lg overflow-hidden transition-all duration-700 ease-out hover:shadow-xl hover:-translate-y-1 ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} 
        style={{ transitionDelay: '200ms' }}
      >
        <div className="relative p-8 md:p-12">
          <CardHeader className="text-center p-0 mb-6">
            <CardTitle className="text-3xl md:text-4xl font-bold text-primary">
              {sozlesmeData.title[language]}
            </CardTitle>
          </CardHeader>
        </div>
      </Card>
      ...
      {/* Ana içerik */}
      <div 
        className={`grid grid-cols-1 gap-8 transition-all duration-700 ease-out ${isLoaded ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-10'}`} 
      ...
      >
        ...
        <Card 
          ...
          style={{ transitionDelay: `${(sozlesmeData.sections.length + 4) * 100}ms` }}
        >
          <CardContent className="p-6 md:p-8">
            <div className="prose prose-lg max-w-none">
              <h3 className="text-xl font-semibold mb-4 text-primary">
                {language === "tr" ? "Önemli Bilgilendirme" : "Important Information"}
              </h3>
              <p>
                {language === "tr" 
                  ? "Bu sözleşme, villa kiralama sürecinde taraflar arasındaki hak ve yükümlülükleri açıklamaktadır. Sözleşme ile ilgili sorularınız için müşteri hizmetlerimizle iletişime geçebilirsiniz."
                  : "This agreement explains the rights and obligations between parties during the villa rental process. For questions regarding the agreement, you can contact our customer service."}
              </p>
              <div className="mt-4">
                <a 
