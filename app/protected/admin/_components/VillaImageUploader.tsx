...
export default function VillaImageUploader({
  ...
  const handleDeleteImage = async (imageId: string) => {
    ...
    try {
      ...
      // Kapak fotoğrafını kontrol et ve güncelle
      const remainingImages = existingImages.filter((img) => img.id !== imageId);
      if (remainingImages.length > 0 && imageData?.isCoverImage) {
      ...
      // Callback çağır
      if (onChange) onChange(remainingImages);
    } catch (error) {
  ...
  const setCoverImage = async (imageId: string) => {
    try {
      ...
      // Callback fonksiyonunu çağır
      if (onChange) {
        onChange(existingImages.map(img => ({
          ...img,
          isCoverImage: img.id === imageId
        })));
      }
            ...
            {existingImages.map((image, index) => (
              <Card key={image.id} className="overflow-hidden">
                <div className="relative aspect-[4/3] bg-muted">
                  ...
                  <div className="absolute top-2 right-2 bg-background/70 text-foreground text-xs px-2 py-1 rounded-md">
                    Sıra: {index + 1}
                  </div>
                </div>
                <CardContent className="p-3">
